...
class vbfhmmProducerPhase2(Module):
...
#        if year == "2017" : self.jesVariation = self.jesVariation + ['pt_jesTimeRunBDown', 'pt_jesTimeRunCDown', 'pt_jesTimeRunDEDown', 'pt_jesTimeRunFDown', 'pt_jesTimeRunBUp', 'pt_jesTimeRunCUp', 'pt_jesTimeRunDEUp', 'pt_jesTimeRunFUp']
    ...
    def mqq(self, jets):
        ...
        j1.SetPtEtaPhiM(jets[0].PT,jets[0].eta, jets[0].phi, jets[0].mass if not self.data and self.year != "2018" else jets[0].mass)
        j2.SetPtEtaPhiM(jets[1].PT,jets[1].eta, jets[1].phi, jets[1].mass if not self.data and self.year != "2018" else jets[1].mass)
        return (j1+j2).M()
    ...
    def analyze(self, event):
        ...
        for lep in muons :
            ...
            muon_corrected_p4.SetPtEtaPhiM(lep.pt , lep.eta, lep.phi, lep.mass)
            lep.iso = lep.pfRelIso04_all
            lep.correctedROC_pt = lep.pt
            #if lep.iso_FSR < 0.8 : 
        ...
        jetFilter4_2017 = lambda j : (j.jetId>0 and (j.PT>50 or j.puId17>0) and abs(j.eta)<4.7 ) and (j.puId17 > 6 or abs(j.eta) < 2.6 or abs(j.eta) > 3.0 )                
        ...
        if not self.data:
            ...
            for j in jetsNolep:
                ...
                for k in jerBinName : setattr(j, k, j.pt if k == "JEReta%spt%s"%(binEta,binPt) else j.pt)
        ...
        for vn in self.jesVariation + jerBinName:
            ...
            jetsCriteria4_2017=[j for j in sortedJets if jetFilter4_2017(j)]
            ...
            if ( len(jetsCriteria4_2017)>=2 and jetsCriteria4_2017[0].PT > 35 and jetsCriteria4_2017[1].PT > 25 and self.mqq(jetsCriteria4_2017) > 250 ) : passAtLeastOne=True
...
vbfhmmModulePhase2 = lambda : vbfhmmProducerPhase2(jetSelection= lambda j : j.pt > 15, muSelection= lambda mu : mu.pt > 9, data = False, year="2026") 
